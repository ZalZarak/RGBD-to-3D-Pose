...
def debug_joints(mode: int, custom_connections=None, connections_except=None, resolution: tuple[int, int] = (480, 270), fps: int = 30, flip: int = 0,
    ...
    if connections_except is not None and custom_connections is not None:
        raise ValueError("Don't input connections_except and custom_connections at the same time")
    ...
    cv2.namedWindow("Debug-Joint")

    values = {}
    if mode == 0:
        values = {k: round((v[0] + v[1]) / 2, 2) for k, v in main.lengths_hr.items()}
    elif mode == 1:
        values = main.depth_deviations.copy()
    ...
    try:
        while key != 27:
            ...
            for pair in connections:
                ...
                value_key = main.joint_map_rev[pair[0]] + "-" + main.joint_map_rev[pair[1]]
                if point1.any() != 0 and point2.any() != 0:
                    ...
                    if mode in (0, 1):
                        ...
                        if mode == 0:
                            l = round(np.linalg.norm(joints_3d[pair[0]] - joints_3d[pair[1]]), 2)
                            values[value_key] = l
                            text = str(l)
                        elif mode == 1:
                            l = round(abs(joints_3d[pair[0], 2] - joints_3d[pair[1], 2]), 2)
                            values[value_key] = l
                            text = str(l)
